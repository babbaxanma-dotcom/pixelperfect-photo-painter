...
(async () => {
  ...
  const totAanHetEind = async (breedte = 1200) => {
  ...
  /* ── 1. helemaal leeg ── */
  {
    const { pg, gepoogd, opEind } = await totAanHetEind();
    if (!opEind) fouten.push('de vragen leidden niet naar het gegevensscherm — de meting telt niet');
    const r = await verstuur(pg, {});
    proeven++;
    if (!r.nogOpFormulier || r.pad !== '/totaalrenovatie') fouten.push('leeg formulier werd toch verstuurd');
  ...
  {
    const { pg } = await totAanHetEind(390);
  ...
  console.log(`check-calculator: ${proeven} verzendpogingen, geen enkele echte lead verstuurd`);
